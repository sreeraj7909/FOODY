...
const Card=({resData})=>{
  ...
  const {areaName,cuisines,avgRating,costForTwo,cloudinaryImageId,name,locality,isOpen,totalRatingsString}=resData?.info
  const {deliveryTime}=resData?.info?.sla
//  const {header,subHeader}=resData?.info?.aggregatedDiscountInfoV3

    return (
          <div className=" w-full md:w-3/12 px-14 md:-m-0 m-8 right-0 left-0 text-white cursor-pointer hover:border-red-300">
               <div className="absolute opacity-90 ">
                 ...
                 <div className="pt-48 mx-2 text-white w-full"> 
